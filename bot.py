...
# Создаем объект бота:
bot = telebot.TeleBot('7296582653:AAEp1vsUma8WeMH1Vu8iZJs-bSqMhWtSbro')
...
def start(message):
    ...
    if database.check_user(user_id):
        ...
        bot.send_message(user_id, 'Выбери пункт меню:', reply_markup=buttons.main_menu(database.get_pr_buttons()))
    else:
...
def get_name(message):
    ...
    bot.send_message(user_id, 'Отлично! Теперь отправь мне свой номер!', reply_markup=buttons.num_button())
...
def choose_count(call):
    ...
    if call.data == 'increment':
        bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.message_id,
                                      reply_markup=buttons.choose_amount(database.get_exact_pr(users[user_id]['pr_name'])[4],
...
def get_location(message, text):
    ...
    if message.location:
        ...
        bot.send_message(8007164255, text)
        bot.send_location(8007164255, latitude=message.location.latitude, longitude=message.location.longitude)
...
def get_num(message, user_name):
    user_id = message.from_user.id
    # Если отправим номер в виде номера
    if message.contact:
...
def choose_pr_count(call):
    ...
    # удаляем сообщение с выыбором в меню
    bot.delete_message(chat_id=user_id, message_id=call.message.message_id)
...
def admin(message):
    if message.from_user.id == 8007164255:
